...
type
  ...
  private
    ...
    function CheckDel(id_sb:integer):boolean;
...
procedure TspisSB.FormCreate(Sender: TObject);
begin
  Fexit:=true;
end;

function TspisSB.savechange: boolean;
begin
  result:=false;
  try
         ...
         //Если добавление
          ...
          begin
              sqlquery1.FieldByName('ID').Value :=CreateGUID('GEN_DEL_SB_ID', sqlquery1.DataBase);
              sqlquery1.FieldByName('id_sb').AsInteger:=sqlquery2.FieldByName('ID').AsInteger;
    ...
    on E: Exception do
      begin
           result:=false;
           if (pos('del_mni_incorrect_operation',lowercase( e.message))>0) then
            begin
...
function TspisSB.Check_(id: integer): integer;
var qq:TSQLQuery;
begin
  result:=-1;
  ...
  qq.SQL.Clear;
  qq.SQL.Add('select * from check_sb('+inttostr(id)+')');
  qq.Open;
...
function TspisSB.CheckDel(id_sb: integer): boolean;
var res:integer;
  sMsg:string;
begin
  res:=self.Check_(id_sb);
  sMsg:='';
  result:=(res=0);
  if res>=2 then
   begin
     sMsg:=sMsg+'системник связан с МНИ';
     res:=res-2;
   end;
  if res>=1 then
   begin
     if length(sMsg)>0 then sMsg:=sMsg+lineending;
     sMsg:=sMsg+'системник распределен';
     res:=res-1;
   end;
  if length(sMsg)>0 then
   begin
     if length(sMsg)>0 then sMsg:=sMsg+lineending;
     sMsg:='системник не может быть списан:'+lineending+sMsg ;
     messagedlg('списание МНИ',sMsg,mtWarning, [mbok],0);
   end;
end;

function TspisSB.SetConnect(connector: TMyworkbd; tipQuery: TDataSetState;
...
begin
  ...
  //Проверим можно ли списывать системник
  //*****************
  ...
  //Откроем наборы
  //***************
  sqlquery2.ParamByName('ID_SB').AsInteger:=editing_id;
...
//  begin
    sqlquery7.ParamByName('id_sb').AsInteger:=editing_id;
    sqlquery7.Open;
 // end;
  ...
  dsInsert:
    begin
         //Вставка
      self.Caption:='списание системника';//Заголовок
...
procedure TspisSB.Button5Click(Sender: TObject);
begin
  Fexit:= savechange;
end;

procedure TspisSB.AaddingExecute(Sender: TObject);
var od:topendialog;
  fname:string;
  qq:tsqlquery;
  SCRIPT:TSQLScript;
...
procedure TspisSB.ADeletingExecute(Sender: TObject);
...
procedure TspisSB.AsaveingExecute(Sender: TObject);
var sd:tsavedialog;
begin
  if sqlquery7.IsEmpty then exit;
...
procedure TspisSB.Button2Click(Sender: TObject);
var
  nRec:integer;
begin
   nRec:=-1;
   nRec:=mainf.openMOTIVESPISSB(true);
   sqlquery3.Refresh;
  if nRec=-1 then exit;
  self.SQLQuery1.FieldByName('ID_MOTIVE_DELETE_SB').AsInteger:=nRec;
end;

procedure TspisSB.Button8Click(Sender: TObject);
begin
  sqlquery1.Cancel;
end;

procedure TspisSB.FormClose(Sender: TObject; var CloseAction: TCloseAction);
begin
...
procedure TspisSB.FormCloseQuery(Sender: TObject; var CanClose: boolean);
begin
  canclose:=fexit;
  fexit:=true;
end;
