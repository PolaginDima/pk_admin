...
SET GENERATOR GEN_SLOVAR_ID TO 7;
COMMIT;
...
COMMIT;
/*Установим начальное значение генератора*/
SET GENERATOR GEN_START_WORK_ID TO 3;
COMMIT;
...
/*создаем генератор для таблицы ЖУРНАЛ SB для поля ID*/
CREATE GENERATOR GEN_SB_ID;
COMMIT;
/*Установим начальное значение генератора*/
SET GENERATOR GEN_SB_ID TO 0;
COMMIT;
...
COMMIT;
/*Установим начальное значение генератора*/
SET GENERATOR GEN_DEL_MNI_ID TO 2;
COMMIT;
...
/*создаем генератор для таблицы ЖУРНАЛ КРИПТОСРЕДСТВ для поля ID*/
CREATE GENERATOR GEN_KRIPTO_ID;
COMMIT;
/*Установим начальное значение генератора*/
SET GENERATOR GEN_KRIPTO_ID TO 0;
COMMIT;

/*создаем генератор для таблицы ЖУРНАЛ КРИПТОСРЕДСТВ для поля ID*/
CREATE GENERATOR GEN_DEL_KRIPTO_ID;
COMMIT;
/*Установим начальное значение генератора*/
SET GENERATOR GEN_DEL_KRIPTO_ID TO 0;
COMMIT;
...
/*создаем исключение для случая уничтожение раньше чем зарегистрировано*/
CREATE EXCEPTION del_mni_incorrect_operation 'This is mni incorrect deleting';
COMMIT;

/*создаем исключение для случая уничтожение раньше чем зарегистрировано*/
CREATE EXCEPTION term_ck_incorrect_operation 'This is kripto incorrect terminate';
COMMIT;

/*создаем исключение для случая криптосредство регистрируется на человека раньше чем он принят на работу*/
CREATE EXCEPTION dt_ck_incorrect_operation 'This is kripto incorrect registration';
COMMIT;

/*Создаем исключения*/
/*создаем исключение для случая дублирование криптосредства*/
