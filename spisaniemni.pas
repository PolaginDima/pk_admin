...
procedure TspisMni.FormCreate(Sender: TObject);
begin
  Fexit:=true;
end;

function TspisMni.savechange: boolean;
begin
  result:=false;
  try
         ...
         //Если добавление
          ...
          begin
              sqlquery1.FieldByName('ID').Value :=CreateGUID('GEN_DEL_MNI_ID', sqlquery1.DataBase);
              sqlquery1.FieldByName('id_mni').AsInteger:=sqlquery2.FieldByName('ID').AsInteger;
...
function TspisMni.CheckDel(id_sw: integer): boolean;
var res:integer;
  sMsg:string;
begin
  ...
  if res>=2 then
   begin
     sMsg:=sMsg+'МНИ связан с системным блоком';
     res:=res-2;
   end;
  if res>=1 then
   begin
     if length(sMsg)>0 then sMsg:=sMsg+lineending;
     sMsg:=sMsg+'МНИ распределен';
     res:=res-1;
   end;
  if length(sMsg)>0 then
   begin
     if length(sMsg)>0 then sMsg:=sMsg+lineending;
     sMsg:='МНИ не может быть списан:'+lineending+sMsg ;
     messagedlg('списание МНИ',sMsg,mtWarning, [mbok],0);
   end;
end;

function TspisMni.Check_(id: integer): integer;
var qq:TSQLQuery;
begin
  result:=-1;
  ...
  qq.SQL.Clear;
  qq.SQL.Add('select * from check_mni('+inttostr(id)+')');
  qq.Open;
...
function TspisMni.SetConnect(connector: TMyworkbd; tipQuery: TDataSetState;
...
begin
  ...
  //Проверим можно ли списывать МНИ
  //*****************
  ...
  //Откроем наборы
  //***************
  sqlquery2.ParamByName('ID_MNI').AsInteger:=editing_id;
 ...
 // begin
    sqlquery7.ParamByName('id_mni').AsInteger:=editing_id;
    sqlquery7.Open;
  //end;
...
procedure TspisMni.Button5Click(Sender: TObject);
begin
  Fexit:= savechange;
end;

procedure TspisMni.AaddingExecute(Sender: TObject);
var od:topendialog;
  fname:string;
  qq:tsqlquery;
  SCRIPT:TSQLScript;
...
procedure TspisMni.ADeletingExecute(Sender: TObject);
...
procedure TspisMni.AsaveingExecute(Sender: TObject);
var sd:tsavedialog;
begin
  if sqlquery7.IsEmpty then exit;
...
procedure TspisMni.Button2Click(Sender: TObject);
var
  nRec:integer;
begin
   nRec:=-1;
   nRec:=mainf.openMOTIVESPISMNI(true);
   sqlquery3.Refresh;
  if nRec=-1 then exit;
  self.SQLQuery1.FieldByName('ID_MOTIVE_DELETE_MNI').AsInteger:=nRec;
end;

procedure TspisMni.Button8Click(Sender: TObject);
begin
  sqlquery1.Cancel;
end;

procedure TspisMni.FormClose(Sender: TObject; var CloseAction: TCloseAction);
begin
...
procedure TspisMni.FormCloseQuery(Sender: TObject; var CanClose: boolean);
begin
  canclose:=fexit;
  fexit:=true;
end;
