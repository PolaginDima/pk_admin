...
//  with  DBTemplateForm do
 // begin
  ...
  //DBTemplateForm.dbgr.DataSource.DataSet.Fields.Fields[0].ReadOnly:=true;
  DBTemplateForm.dbgr.Align:=alClient;//alTop;
  ...
  {for i:=0 to DBTemplateForm.dbgr.Columns.Count-1 do
  DBTemplateForm.dbgr.Columns.Items[i].ReadOnly:=true;}
  ...
  DBTemplateForm.dbgr.Visible:=true;
   DBTemplateForm.dbgr.Options:=DBTemplateForm.dbgr.Options+[dgRowHighlight];//Подсветка выделенной строки
  ...
  DBTemplateForm.dbgr.SoftFind:=true;
...
begin
  ...
  setlength(stqrv,self.qrrV.FieldCount);
           ...
           qrr.Open;
           qrrV.Open;
           if not qrrV.Locate('ID',i,[]) then
           begin
                if (not qrrV.IsEmpty)and(qrrV.RecordCount>1) then
                begin
                     if (recn<=qrrV.RecordCount) then
                        qrrV.RecNo:=recn else
                          qrrV.RecNo:=recn-1;
                end;
           end;
            for j:=0 to self.qrrV.FieldCount-1 do
...
begin
  ...
  begin
    ...
    begin
      ...
      //showmessage(lookup.DataField);
      //ds.DataSet.Fields[i].LookupKeyFields);
      lookup.KeyField:=qr.Fields[i].LookupKeyFields;
      //showmessage(lookup.KeyField);
      lookup.ListField:=qr.Fields[i].LookupResultField;
      //showmessage(lookup.ListField);
      //dstmp.DataSet:=nil;
      //dstmp:=nil;
      J:=J+1;
      continue;
  ...
  {  if ds.DataSet.Fields.Fields[i].FieldKind<>fkCalculated then
   ...
   // for i:=0 to DBEditSlovar.Panel1.ComponentCount-1 do    DBEditSlovar.Panel1.Components[i].Free;
    ...
    begin
    try
      for i:=0 to DBEditSlovar.ControlCount-1 do
          if (DBEditSlovar.Controls[i] is tdblookupcombobox) and ((DBEditSlovar.Controls[i] as tdblookupcombobox).DataSource<>nil) then
             (DBEditSlovar.Controls[i] as tdblookupcombobox).DataSource.Free;
      Free;
      finally
      end;
     end;
