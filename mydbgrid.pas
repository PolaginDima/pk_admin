...
uses
  Classes, SysUtils,DBGrids,Grids,Graphics, StdCtrls,ExtCtrls,lcltype,{lclproc,}db,
  Dialogs, Controls, sqldb, lazutf8;
type
 ...
 // DataSource.DataSet.DisableControls;
  ...
  for i:=0 to high(Filter)  do
    begin
         ...
         if DataSource.DataSet.Filtered then break;
    end;
  ...
  inherited DrawCell(aCol, aRow, aRect, aState);
    ...
    F := GetFieldFromGridColumn(aCol);
      ...
      begin
        ...
        if (utf8pos(FColArray[aCol].name,SQLQUERYdop.IndexFieldNames)>0) then
        begin
...
begin
 ...
 begin
  ...
  begin
    SQLQUERYdop.IndexFieldNames:='';
  end else
  for i:=0 to high(self.Fcolarray) do
       if (Fcolarray[i].left+1<x) and(Fcolarray[i].right-1>x) then
           begin
             if ssShift in Shift then
             begin
               if length(SQLQUERYdop.IndexFieldNames)<=0 then
                  SQLQUERYdop.IndexFieldNames:=Fcolarray[i].name else
                  SQLQUERYdop.IndexFieldNames:=SQLQUERYdop.IndexFieldNames+';'+Fcolarray[i].name;
             end else
                 begin
                      SQLQUERYdop.IndexFieldNames:=Fcolarray[i].name;
                      //bm:=TBookmark.create;
                      //bm:=SQLQUERYdop.GetBookmark;
                      //SQLQUERYdop.Close;

                      {
                      SQLQUERYdop.Active:=false;

                      SQLQUERYdop.AddIndex(Fcolarray[i].name,Fcolarray[i].name,[]{,Fcolarray[i].name});

                      SQLQUERYdop.Active:=true;

                      SQLQUERYdop.IndexName:=Fcolarray[i].name;
                      }

                      //SQLQUERYdop.Bookmark:=bm;

                      //SQLQUERYdop.Open;
                     { SQLQUERYdop.Active:=false;
                      if SQLQUERYdop.IndexDefs.IndexOf(Fcolarray[i].name)<0 then
                      begin
                           SQLQUERYdop.ClearIndexes;
                           idef:=SQLQUERYdop.IndexDefs.AddIndexDef;
                           idef.Name:=Fcolarray[i].name;
                           idef.Fields:=Fcolarray[i].name;
                           idef.Options:=[ixDescending];
                           //SQLQUERYdop.AddIndex(Fcolarray[i].name,Fcolarray[i].name,[],'','');
                           //SQLQUERYdop.Open;
                      end;
                      SQLQUERYdop.Active:=true;
                      //SQLQUERYdop.Bookmark:=bm;
             ...
             {showmessage(inttostr(SQLQUERYdop.IndexDefs.Count));
             showmessage(SQLQUERYdop.IndexDefs.Items[0].Name);  }
             break;
           end;
 end;
end;
